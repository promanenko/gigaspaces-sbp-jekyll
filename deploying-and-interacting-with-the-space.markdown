...
If you are connecting to the Space from a remote JVM, you can use all of the above methods, with the exception that [the Space URL]({%latestjavaurl%}/the-space-configuration.html) you should use is a remote URL (using the `jini://` discovery protocol), and you don't have to specify the clustering topology or the instance ID (since your code in that case does not create the Space, it simply connects to an already running Space).
...
{% refer %}Refer to [Change API]({%latestjavaurl%}/change-api.html) for full details regarding the change operation{% endrefer %}
